...
}> = ({ label, items, template, render }) => (
  <div
    className="gap-2 py-2.5 border-t border-slate-100"
...
const ComparePanel: React.FC<Props> = ({ items, onClose, onRemove }) => {
  ...
  const template = `5.5rem repeat(${items.length}, minmax(8.5rem, 1fr))`;
  const minWidth = `${5.5 + items.length * 8.5}rem`;
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in slide-in-from-bottom-4 duration-300"
          ...
          <div className="flex items-center space-x-3 pr-8">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                {items.length} drugs side by side{items.length > 3 ? ' — scroll horizontally →' : ''}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 py-4 pb-[calc(env(safe-area-inset-bottom)+1.5rem)]">
          <div className="overflow-x-auto -mx-1 px-1">
            <div style={{ minWidth }}>
              ...
              <div className="gap-2" style={{ display: 'grid', gridTemplateColumns: template }}>
                <div />
                {items.map((d) => (
              ...
              <div className="mt-1">
                <CompareRow label="EU (EMA)" items={items} template={template} render={(d) => euStatus(d)} />
                <CompareRow label="US (FDA)" items={items} template={template} render={(d) => usStatus(d)} />
                <CompareRow label="Company" items={items} template={template} render={(d) => d.company || '—'} />
                <CompareRow label="Class" items={items} template={template} render={(d) => d.drugClass || '—'} />
                <CompareRow
                  label="Area"
                  items={items}
                  template={template}
                ...
                />
                <CompareRow label="Flags" items={items} template={template} render={(d) => flagLabels(d)} />
                <CompareRow
                  label="At a glance"
                  items={items}
                  template={template}
                ...
                />
                <CompareRow label="Indication" items={items} template={template} render={(d) => d.indication || '—'} />
              </div>
            </div>
          </div>
